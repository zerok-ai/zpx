...
if [ "$CREATE_ORG" == "1" ]
then
  curl "https://work.$PX_DOMAIN/api/graphql" \
    -H "authority: work.$PX_DOMAIN" \
    -H 'accept: */*' \
    -H 'accept-language: en-GB,en;q=0.9' \
    -H 'content-type: application/json' \
...
## Curl 11
log "Curl 11"
log "export PX_DOMAIN=$PX_DOMAIN"
log "export csrf_token_dc395c08d3e90de40ef4c0d97c76075e6c9c0500d9dc68488cfc94dfdfa5e5a8=$csrf_token_dc395c08d3e90de40ef4c0d97c76075e6c9c0500d9dc68488cfc94dfdfa5e5a8"
log "export oauth2_authentication_csrf=$oauth2_authentication_csrf"
log "export ory_kratos_session=$ory_kratos_session"
log "export ossidprovider=$ossidprovider"
log "export oauth2_consent_csrf=$oauth2_consent_csrf"
log "export default_session5=$default_session5"
log "export access_token=$access_token"
curl -s "https://work.$PX_DOMAIN/api/graphql" \
  -H "authority: work.$PX_DOMAIN" \
  -H 'accept: */*' \
  -H 'accept-language: en-GB,en;q=0.9' \
...
access_key_id=$(cat $TMP_DIR/output11.txt | sed 's/"id"/\n"id"/g' | sed 's/,/\n,/g' | grep "\"id\"" | sed 's/"//g' | sed 's/id://g')
...
access_key=$(cat $TMP_DIR/output12.txt | sed 's/"key"/\n"key"/g' | sed 's/,/\n,/g' | grep "\"key\"" | sed 's/"//g' | sed 's/key://g')
log "$access_key"
echo $access_key
# rm -rf $TMP_DIR
